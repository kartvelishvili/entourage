...
const AdminResults = () => {
  ...
  const fetchItems = async () => {
    const data = await api('/api/content/results');
    setItems(data);
    setLoading(false);
  };
  ...
  const handleSave = async () => {
    try {
      if (editing.id) {
        await api(`/api/admin/results/${editing.id}`, { method: 'PUT', body: editing });
      } else {
        await api('/api/admin/results', { method: 'POST', body: editing });
      }
      setEditing(null);
      fetchItems();
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white mb-1">შედეგები (Before/After)</h1>
          <p className="text-gray-500">{items.length} შედეგი</p>
        </div>
        <button onClick={() => setEditing({ title: '', description: '', before_image: '', after_image: '' })} className="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2.5 rounded-xl font-medium flex items-center gap-2">
        ...
        {items.map(item => (
          <div key={item.id} className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
            <div className="grid grid-cols-2 h-32">
              <div className="bg-gray-800">{item.before_image && <img src={item.before_image} alt="Before" className="w-full h-full object-cover" />}</div>
              <div className="bg-gray-800">{item.after_image && <img src={item.after_image} alt="After" className="w-full h-full object-cover" />}</div>
            </div>
            <div className="p-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
          <div className="bg-gray-900 border border-gray-800 rounded-2xl w-full max-w-lg shadow-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800">
              <h2 className="text-lg font-bold text-white">{editing.id ? 'რედაქტირება' : 'ახალი შედეგი'}</h2>
            ...
            <div className="p-6 space-y-4">
              ...
              <ImageUpload label="Before სურათი" value={editing.before_image || ''} onChange={url => setEditing({ ...editing, before_image: url })} />
              <ImageUpload label="After სურათი" value={editing.after_image || ''} onChange={url => setEditing({ ...editing, after_image: url })} />
            </div>
